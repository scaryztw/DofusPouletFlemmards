...
class TabEchangeMixin:
    def _tab_trade(self):
        ...
        lay.addWidget(self._note(
            ...
            "Les fenêtres doivent être empilées au même endroit (positions partagées entre persos)."))
        ...
        lay.addWidget(sep)
        lay.addWidget(QLabel("Détection par capture d'une petite zone de l'écran (comme le script "
                             "d'origine) — indépendant des notifications, donc immunisé au bug des "
                             "échanges répétés. Dès qu'un de tes pseudos est vu dans la zone (popup "
                             "« En attente de la réponse de … »), DPF bascule sur SA fenêtre, clique "
                             "ta Position « Oui » ci-dessus, puis revient au lanceur."))
        ...
        lay.addLayout(g)
        lay.addWidget(self._note(
            "Clique « Sélectionner la zone », puis maintiens le clic et glisse pour entourer le NOM "
            "dans « En attente de la réponse de <pseudo> » du popup d'échange — comme une capture "
            "d'écran (Échap pour annuler). Prends LARGE : le texte est centré, donc la position du nom "
            "change selon sa longueur."))
        ...
        lay.addLayout(rt)
        lay.addWidget(self._note(
            "Place un <pseudo>.png par perso dans le dossier SCREENSHOT-ECHANGE (mêmes fichiers que la "
            "version AHK). « Recharger les pseudos » : relit ces fichiers. « Ouvrir le dossier » : "
            "ouvre SCREENSHOT-ECHANGE. « Diagnostic A-Trade » : pendant un échange, écrit dans l'onglet "
            "Debug ce que DPF voit (zone du nom + score de chaque pseudo)."))
